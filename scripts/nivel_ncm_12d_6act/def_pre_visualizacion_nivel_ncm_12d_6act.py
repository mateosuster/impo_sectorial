...
def def_top_cuits(asign_pre_matriz, dic_propio, bien, cuits_desc ):
    top_cuits= asign_pre_matriz.groupby(["cuit", "sd"], as_index=False)['valor_pond'].sum("valor_pond").sort_values("valor_pond", ascending=False)#.iloc[0:50]
...
def def_top_cuit_de_top_hs(asign_pre_matriz, ncm12_desc, dic_propio, top_productos, bien, cuits_desc):
    prpal_sd = asign_pre_matriz[asign_pre_matriz["hs6_d12"].isin(top_productos["hs6_d12"])].groupby(["cuit", "hs6_d12"], as_index=False)["valor_pond"].sum().sort_values( "valor_pond", ascending = False)
    prpal_sd  = prpal_sd .groupby(["cuit","hs6_d12"], as_index = False).head(10).sort_values(["hs6_d12", "valor_pond"], ascending=False)
    prpal_sd  = pd.merge(left=prpal_sd  , right=ncm12_desc, left_on="hs6_d12", right_on="HS_12d", how="left").drop("HS_12d", axis=1)
